...
import com.kai.lktMode.fragment.MainActivity;
...
public class UpdateUtil {
        ...
        new Thread(new Runnable() {
            ...
            public void run() {
                try {
                    ...
                    final DavResource resource=sardine.list(website).get(0);
                    if (resource!=null){
    ...
    public static void list(final Activity activity){
        final String[] keys=new String[]{"autoBoot","custom","autoLock","autoClean","imClean","games","softwares"};
        new Thread(new Runnable() {
            ...
            public void run() {
                try {
                    ...
                    Sardine sardine=SardineFactory.begin(user,password);
                    List<DavResource> resources=sardine.list(website+"/lktMode");
                    for (DavResource resource:resources){
                        if (resource.getName().equals("lktMode")||!resource.getName().contains(".zip"))
                            continue;
        ...
        new Thread(new Runnable() {
            ...
            public void run() {
                ...
                try {
                    ...
                    //将云端备份下载到本地
                    Sardine sardine= SardineFactory.begin(user,password);
                    InputStream in=sardine.get(website+"/lktMode/"+webpath);
                    OutputStream out=new FileOutputStream(new File(filesPath+"/restore.zip"));
            ...
            public void run() {
                ...
                try {
                    ...
                    for (String path:paths){
                        sardine.delete(website+"/lktMode/"+path);
                    }
                    activity.runOnUiThread(new Runnable() {
                        @Override
                        public void run() {
    ...
    public static void backup(final String filePath, final Activity context){
        new Thread(new Runnable() {
            ...
            public void run() {
                ...
                try {
                    Sardine sardine= SardineFactory.begin(user,password);
                    sardine.createDirectory(website+"/lktMode/");
                    ...
                    sardine.put(website+"/lktMode/backup-"+year+"-"+month+"-"+day+"-"+hour+"-"+minute+".zip",inputStream);
                    context.runOnUiThread(new Runnable() {
                        @Override
                        public void run() {
